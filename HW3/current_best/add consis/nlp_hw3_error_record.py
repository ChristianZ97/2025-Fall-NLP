...
def consistency_loss(reg_scores, clf_logits):
    ...
    clf_pred = torch.argmax(clf_logits, dim=1)
    neutral_mask = (reg_scores >= 2.0) & (reg_scores < 3.5)
    entail_mask = reg_scores >= 3.5

    loss = torch.zeros_like(reg_scores)

    neutral_inconsistent = neutral_mask & (clf_pred == 0)
    entail_inconsistent = entail_mask & (clf_pred == 1)

    loss += neutral_inconsistent.float() * ((reg_scores - 2.75) ** 2) * 2.0
    loss += entail_inconsistent.float() * ((reg_scores - 4.25) ** 2) * 2.0
...
for ep in range(epochs):
    ...
    for batch in pbar:
        ...
        loss = (
            0.85 * (config.alpha * loss_reg + (1 - config.alpha) * loss_clf)
            + 0.15 * consis_loss
        )
