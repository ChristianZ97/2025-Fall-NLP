...
df_train.loc[mask, "tgt"] = df_train.loc[mask, "tgt"].apply(
    lambda x: str(int(x) + np.random.randint(-50, 51))
)
...
i = 0
print(
    f"LSTM w/ {sum(p.numel() for p in model.parameters() if p.requires_grad)} trainable params + Noise Dataset"
)
