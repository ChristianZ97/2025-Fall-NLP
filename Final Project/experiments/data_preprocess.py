...
def run_processing_pipeline(
    ...
    """
    執行文件處理管線。
    針對每個 Document ID，同時嘗試產出 PDF 版本與 TeX 版本的處理文字。

    輸出結構：
      output_root/
        ├── pdf_texts/   <-- 存放 PDF 解析後的 txt
        ├── tex_texts/   <-- 存放 TeX 解析後的 txt
        ├── pdfs/        <-- 下載的原始 PDF
        └── sources/     <-- 下載的原始 TeX 壓縮檔
    """
    root = Path(output_root)

    # 定義並建立輸出資料夾
    # 這裡直接命名為您要求的 "texts" 分類
    dir_pdf_out = root / "pdf_texts"
    dir_tex_out = root / "tex_texts"

    dir_pdf_out.mkdir(parents=True, exist_ok=True)
    dir_tex_out.mkdir(parents=True, exist_ok=True)
    ...
    print(f"Output PDF texts to: {dir_pdf_out}")
    print(f"Output TeX texts to: {dir_tex_out}")
    ...
    for _, row in df.iterrows():
        ...
        # ==========================================
        # 檢查是否達到 PDF 處理數量上限
        if pdf_limit is not None and processed_pdf_count >= pdf_limit:
            print(f"[PDF] Limit reached, skipping PDF processing.")
        else:
            # 判斷 URL 是否適合 PDF 下載
            if pdf_processor.is_pdf_url(url) or pdf_processor.has_pdf_mime(url):
                # 嘗試下載或取得現有 PDF
                if download_pdf:
                ...
                if pdf_path and pdf_path.exists():
                    try:
                        ...
                        payload = pdf_processor.to_payload(
                            pdf_path, doc_id=doc_id, title=title, metadata=meta
                        )
                        # 儲存文字檔到 pdf_texts 資料夾
                        out_file = dir_pdf_out / f"{doc_id}.txt"
                        out_file.write_text(payload.text, encoding="utf-8")

                        print(f"[ok] PDF Text -> {out_file.name}")
                        processed_pdf_count += 1
                    except Exception as e:
                        print(f"[error] PDF processing failed for {doc_id}: {e}")
                else:
                    print(f"[skip] PDF not found or download failed.")
            else:
                print(f"[skip] URL is not a valid PDF.")

        # ==========================================
        # 2. 處理 TeX 版本
        # ==========================================
        # 若要處理 TeX，必須確保有下載 Source
        if download_source:
            # 下載或取得現有 Source
                ...
                if main_tex:
                    try:
                        ...
                        payload = tex_processor.to_payload(
                            main_tex, doc_id=doc_id, title=title, metadata=meta
                        )
                        # 儲存文字檔到 tex_texts 資料夾
                        out_file = dir_tex_out / f"{doc_id}.txt"
                        out_file.write_text(payload.text, encoding="utf-8")

                        print(f"[ok] TeX Text -> {out_file.name}")
                    except Exception as e:
                        print(f"[error] TeX processing failed for {doc_id}: {e}")
                else:
                    print(f"[skip] TeX: No main .tex file found in source.")
            else:
                print(f"[skip] TeX: Source download failed or not available.")

    print(f"\nPipeline completed.")
    print(f"PDF Texts: {processed_pdf_count} files in {dir_pdf_out}")
    print(f"TeX Texts: (varied) files in {dir_tex_out}")
